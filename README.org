...
** "Launcher"
   ...
   #+begin_src emacs-lisp
     (defcustom jjin-exec-applications nil
       "Mapping between symbolic identifiers and corresponding
     'execution functions.'  Use this mapping to expose functions to
     `jjin-execute' for easy access"
       :type '(alist :key-type symbol :value-type function))

     (defun jjin-execute ()
       "Select an 'application' from ``jjin-exec-applications' and
       execute it."
       (interactive)
       (let* ((prompt "> ")
              (selections (mapcar (lambda (asoc) (symbol-name (car asoc))) jjin-exec-applications))
              (key (intern (completing-read prompt selections nil t))))
         (funcall (alist-get key jjin-exec-applications))))

     (defun helm-jjin-execute-handler (prompt
                                       collection
                                       predicate
                                       require-match
                                       initial-input
     ...
     (with-eval-after-load 'helm
       (add-to-list 'helm-completing-read-handlers-alist '(jjin-execute . helm-jjin-execute-handler)))

     (bind-key "C-c e" 'jjin-execute)
   #+end_src
...
** =calc=
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package calc
       :init
       (add-to-list 'jjin-exec-applications '(calc . calc)))
   #+END_SRC
...
** Elfeed
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package elfeed
       ...
       :init
       (add-to-list 'jjin-exec-applications '(elfeed . elfeed))
     ...
     (use-package elfeed-protocol
       ...
       :straight (:host github
                  :type git
                  :repo "jinnovation/elfeed-protocol"
                  :branch "canary"
                  :upstream (:host github :repo "fasheng/elfeed-protocol"))
...
** =mu4e=
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package mu4e
       ...
       :init
       (add-to-list 'jjin-exec-applications '(mu4e . mu4e))
       ...
       (setq mu4e-get-mail-command
             (if (not (executable-find "offlineimap"))
                 (progn
                   (use-package-error "mu4e: binary offlineimap not found; `mu4e-get-mail-command' falling back to no-op.")
                   "true")
               "offlineimap"))
...
** =w3m=
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package w3m
       :commands w3m
       :init
       (add-to-list 'jjin-exec-applications '(w3m . w3m))
